...
class UserValidate
{
    ...
    public function updatePassword($req)
    {
        ...
        if (!isset($req['oldPassword']) || trim($req['oldPassword']) == '') {
            array_push($msg, 'Please fill out old password');
        }
        if (!isset($req['newPassword']) || trim($req['newPassword']) == '') {
            array_push($msg, 'Please fill out new password');
        }
        if (isset($req['newPassword']) && $req['newPassword'] != $req['confirmNewPassword']) {
            array_push($msg, 'Password is not matched');
        }
        return $msg;
    }
